...
///普通難易度の最後の敵の弾の動きの処理２
///</summary>
public class NormalStageLastEnemyBulletSecond : MonoBehaviour
{
    const float rotationTime = 7f;
    const float downTime = 1.8f;
    const float waitTime = 1.6f;
    const float riseTime = 3f;
    const float downYPosition = -2.2f;
    const float riseYPosition = 8f;
    const float rotateX = 0f;
    const float rotateY = 0f;
    const float rotateZ = -360f;

    ///<summary>
    ///右回転しながら下に進み、しばらくしてから上に戻っていく
    ///</summary>
    void Start()
    {
